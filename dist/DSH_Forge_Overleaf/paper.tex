...
Every queued item carries its score inputs and the reason for its discovery position. Forge exposes two evaluation modes. The first compares quality-only, popularity, and recency orderings over one fixed candidate set. The second selects separate arms from a common admissible pool, takes their union, removes stars, update dates, scores, visibility labels, and arm membership, then places the arm key in a separate file. A planning command records the assumptions behind the requested arm size. The current default assumptions produce 97 candidates per arm, while the protocol reserves smaller arms for a pilot.

Forge assigns each candidate to a fixed number of reviewers while balancing individual and reviewer-pair loads. Each reviewer receives a self-contained HTML file that makes no network requests, stores work locally, and exports a snapshot-bound ledger. Reviewers can abstain when evidence or expertise is insufficient; an abstention is recorded but does not become a negative label or satisfy the minimum scored-review count. The merge step detects conflicting duplicate ratings. The benchmark refuses incomplete review coverage, aggregates independent ratings, and reports per-arm precision with bootstrap intervals, NDCG, low-visibility yield, owner diversity, Forge-minus-baseline differences, exact agreement, and quadratic weighted kappa. This makes a controlled selection study reproducible, although it does not supply the human judgments. Curator feedback never authorizes installation.
...
Does the implementation satisfy its tested contracts? & 207 Python and 43 JavaScript tests passed & Strong regression evidence for covered behavior, not a live multi-platform acceptance test.\\
Can selection methods be compared without showing rank signals? & Blinded four-arm study, balanced reviewer packets, sealed key, and strict benchmark & The experiment mechanics are implemented and snapshot-bound. Human ratings and statistical results remain outstanding.\\
...
The primary outcome should be independently judged relevance at $k$, with ``promising'' and ``exceptional'' treated as relevant. NDCG captures graded relevance, while review coverage and ordinal agreement expose annotation quality. Under the documented planning assumptions, the initial calculation returns 97 candidates per arm. A separate pilot should refine that calculation before the confirmatory snapshot is frozen. The implemented commands enforce snapshot and subject binding, assign exact review overlap, detect conflicting reviewer exports, reject a modified ballot or answer key, and refuse to benchmark an incomplete ledger. The resulting report includes deterministic artifact-level bootstrap intervals and makes arm overlap explicit. These controls do not prevent a researcher from disclosing the key or choosing a favorable sample after seeing results.\cite{studyprotocol}

\subsection{Operational evidence}

Continuous integration exercises catalog behavior, metadata validation, launcher policy, interface behavior, and preview generation. The pinned successful run reported 207 Python and 43 JavaScript tests. It does not exercise every browser, operating system, real Apptainer deployment, package, or model-backed interaction.\cite{ci,catalogci}
